...
public class StreamController {
     ...
     *
     */
    @PostMapping("/getStreamBySearch/{current}/{limit}")
    public Result getStreamBySearch(@PathVariable int current,
                                    @PathVariable int limit,
                                    @RequestBody(required = false) StreamVo vo){
        Page<Stream> page = PageHelper.startPage(current,limit);
        List<Stream> list = streamService.getStreamBySearch(vo);
        long total = page.getTotal();
        return Result.ok().data("streams",list).data("total",total);
    }
}
